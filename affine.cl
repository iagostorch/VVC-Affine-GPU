    ...
    // Starts the motion estimation based on gradient and optical flow
     for(int iter=0; iter<numGradientIter+1; iter++){ // +1 because we need to conduct the initial prediction (with AMVP) in addition to the gradient-ME
        ...
        for(int pass=0; pass<4; pass++){   
            ...
            // TODO: The (int2)(cuX+sub_X,cuY+sub_Y) is deprecated and not used anymore. Correct it.
        ...
        // TODO: These memory accesses are not optimized and they are likely to demand HUGE processing time
        ...
        if(lid%itemsPerCu==0){
            ...
            for(int col=0; col<cuWidth; col++){
                horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + col] = horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + col+CTU_WIDTH];
                horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + col+(cuHeight-1)*CTU_WIDTH] = horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + col+(cuHeight-2)*CTU_WIDTH];

                verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + col] = verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + col+CTU_WIDTH];
                verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + col+(cuHeight-1)*CTU_WIDTH] = verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + col+(cuHeight-2)*CTU_WIDTH];
            }
            ...
            for(int row=0; row<cuHeight; row++){
                horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + row*CTU_WIDTH] = horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + row*CTU_WIDTH + 1];
                horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + row*CTU_WIDTH+cuWidth-1] = horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + row*CTU_WIDTH+cuWidth-2];

                verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + row*CTU_WIDTH] = verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + row*CTU_WIDTH + 1];
                verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + row*CTU_WIDTH+cuWidth-1] = verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + row*CTU_WIDTH+cuWidth-2];
            }
            // Fills the four corners
            horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + 0] = horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + CTU_WIDTH + 1];
            verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + 0] = verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + CTU_WIDTH + 1];
            
            horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + cuWidth-1] = horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + CTU_WIDTH+cuWidth-2];
            verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + cuWidth-1] = verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + CTU_WIDTH+cuWidth-2];
            
            horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + (cuHeight-1)*CTU_WIDTH] = horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + (cuHeight-2)*CTU_WIDTH+1];
            verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + (cuHeight-1)*CTU_WIDTH] = verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + (cuHeight-2)*CTU_WIDTH+1];
            
            horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + (cuHeight-1)*CTU_WIDTH+cuWidth-1] = horizontalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + (cuHeight-2)*CTU_WIDTH+cuWidth-2];
            verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + (cuHeight-1)*CTU_WIDTH+cuWidth-1] = verticalGrad[wg*CTU_WIDTH*CTU_HEIGHT + cuY*CTU_WIDTH+cuX + (cuHeight-2)*CTU_WIDTH+cuWidth-2];        
        }
        ...
        // !!!!!!!!!!!!!!!!!!!!!!!!!
        // !!!!!!!!!!!!!!!!!!!!!!!!!
        // TODO: IMPORTANT: This can be improved. In the current implementation, we will need itemsPerCu warps executing simultaneously, each one with a single active workitem. If we use if(lid<itemsPerCu) and then multiply lid*itemsPerCu, it is possible to use all workitems of the same warp and avoid divergent branching on the same warp
        // TODO: this idea allows using fewer resources to obtain the same result
        // 
        if(lid%itemsPerCu==0){
            ...
            for(int col=0; col<7; col++){
                for(int row=0; row<6; row++){
                    ...
                    private_pEqualCoeff[col][row] = global_pEqualCoeff[wg*256*7*6 + lid*7*6 + col*6 + row];
                }
            }
            ...
            for(int item=1; item<itemsPerCu; item++){
                ...
                for(int col=0; col<7; col++){
                    for(int row=0; row<6; row++){
                        private_pEqualCoeff[col][row] += global_pEqualCoeff[wg*256*7*6 + (lid+item)*7*6 + col*6 + row];
                    }
                }
            ...
            /*
            if(wg==targetWg && lid%itemsPerCu==0 && cuIdx==targetCuIdx){
                printf("Iter: %d WG: %d Width: %d Height: %d X: %d Y: %d cuIdx: %d \n", iter, wg, cuWidth, cuHeight, ctuX+cuX, ctuY+cuY, cuIdx);
                printf("\tCurrent CPMVs.       LT: %dx%d        RT: %dx%d    isSpread: %d\n", LT_X[cuIdx], LT_Y[cuIdx], RT_X[cuIdx], RT_Y[cuIdx], isSpread);
            }
            //*/

            // Update the current MVs and clip to allowed values
            LT_X[cuIdx] += intDeltaMv.s0;
            LT_Y[cuIdx] += intDeltaMv.s1;
            RT_X[cuIdx] += intDeltaMv.s2;
            RT_Y[cuIdx] += intDeltaMv.s3;
            LB_X[cuIdx] += intDeltaMv.s4;
            LB_Y[cuIdx] += intDeltaMv.s5;

            LT_X[cuIdx] = clamp(LT_X[cuIdx], MV_MIN, MV_MAX);
            LT_Y[cuIdx] = clamp(LT_Y[cuIdx], MV_MIN, MV_MAX);
            RT_X[cuIdx] = clamp(RT_X[cuIdx], MV_MIN, MV_MAX);
            RT_Y[cuIdx] = clamp(RT_Y[cuIdx], MV_MIN, MV_MAX);
            LB_X[cuIdx] = clamp(LB_X[cuIdx], MV_MIN, MV_MAX);
            LB_Y[cuIdx] = clamp(LB_Y[cuIdx], MV_MIN, MV_MAX);
            
            int2 tmp_mv;

            tmp_mv = clipMv((int2)(LT_X[cuIdx],LT_Y[cuIdx]),ctuX+cuX, ctuY+cuY, cuWidth, cuHeight, frameWidth, frameHeight);
            LT_X[cuIdx] = tmp_mv.s0;
            LT_Y[cuIdx] = tmp_mv.s1;
            tmp_mv = clipMv((int2)(RT_X[cuIdx],RT_Y[cuIdx]),ctuX+cuX, ctuY+cuY, cuWidth, cuHeight, frameWidth, frameHeight);
            RT_X[cuIdx] = tmp_mv.s0;
            RT_Y[cuIdx] = tmp_mv.s1;
            tmp_mv = clipMv((int2)(LB_X[cuIdx],LB_Y[cuIdx]),ctuX+cuX, ctuY+cuY, cuWidth, cuHeight, frameWidth, frameHeight);
            LB_X[cuIdx] = tmp_mv.s0;
            LB_Y[cuIdx] = tmp_mv.s1;      

            // Export progress for a given WG
            /*
            if(wg==targetWg && lid%itemsPerCu==0 && cuIdx==targetCuIdx){
                printf("\tCurrent deltas. deltaLT: %dx%d   deltaRT: %dx%d\n", intDeltaMv.s0, intDeltaMv.s1, intDeltaMv.s2, intDeltaMv.s3);
                printf("\tUpdated CPMVs.       LT: %dx%d        RT: %dx%d\n", LT_X[cuIdx], LT_Y[cuIdx], RT_X[cuIdx], RT_Y[cuIdx]);
                printf("\n-----\n");
            }
            //*/
...
__kernel void affine_gradient_128x128(__global int *referenceFrameSamples, __global int *currentFrameSamples,const int frameWidth, const int frameHeight, __global short *horizontalGrad, __global short *verticalGrad, __global long *global_pEqualCoeff, __global long *gBestCost, __global int *gBestLT_X, __global int *gBestLT_Y, __global int *gBestRT_X, __global int *gBestRT_Y, __global int *gBestLB_X, __global int *gBestLB_Y,  __global long *debug, __global short *retCU){
    ...
    int16 deltaHorVec, deltaVerVec, predBlock, original_block;
    int bipred=0, windowWidth=11, windowHeight=11, referenceWindow[11*11], satd;
    // TODO: Beware that VTM has a fast decision method that may not use PROF in smaller block sizes even when PROF=1
    int enablePROF=0; // Enable or disable PROF after filtering (similar to --PROF=0/1)
    long cumulativeSATD;
    int bitrate = MAX_INT;
    int numGradientIter = 5; // Number of iteration in gradient ME search (i.e., number of CPMV updates after predicted MV)
    ...
    for(int iter=0; iter<numGradientIter+1; iter++){ // +1 because we need to conduct the initial prediction (with AMVP) in addition to the gradient-ME
        ...
        if(lid==0){
            ...
            // TODO: This "+4" represents the ruiBits of the VTM-12.0 encoder, and it is the base-bitrate for using affine. The "+4" when using low delay with a single reference frame. Improve this when using multiple reference frames
            currCost = local_cumulativeSATD[0] + (long) getCost(bitrate+4, lambda_QP22);
